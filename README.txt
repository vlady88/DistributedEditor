...
Distributed application that allows simultaneous editing of a piece of text from multiple editors (possibly) running on different machines. 
...
The editor makes use of a server that synchronizes editing commands coming from multiple editor instances. These commands can be of 2 types:
...
For the user interface, a JFrame object with a text are is used. When a character is typed or deleted, the change is displayed to the user only after the command has been sent to the server and acknowledged to make sure that consistency among editors is preserved.

Usage
=====

The project requires Apache Ant. 
